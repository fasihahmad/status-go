...
type WakuMailServer struct {
	...
	symFilter  *wakutypes.Filter
	asymFilter *wakutypes.Filter
}
...
func (s *WakuMailServer) Archive(env *wakutypes.Envelope) {
	s.ms.Archive(gethbridge.NewWakuEnvelope(env))
}

func (s *WakuMailServer) Deliver(peerID []byte, req wakutypes.MessagesRequest) {
	s.ms.DeliverMail(types.BytesToHash(peerID), types.BytesToHash(req.ID), MessagesRequestPayload{
		Lower:  req.From,
		Upper:  req.To,
		Bloom:  req.Bloom,
...
// DEPRECATED; user Deliver instead
func (s *WakuMailServer) DeliverMail(peerID []byte, req *wakutypes.Envelope) {
	payload, err := s.decodeRequest(peerID, req)
	if err != nil {
		deliveryFailuresCounter.WithLabelValues("validation").Inc()
		log.Error(
...
func (s *WakuMailServer) setupDecryptor(password, asymKey string) error {
	...
	if password != "" {
		...
		s.symFilter = &wakutypes.Filter{KeySym: symKey}
	}
	...
	if asymKey != "" {
		...
		s.asymFilter = &wakutypes.Filter{KeyAsym: keyAsym}
	}
...
func (s *WakuMailServer) openEnvelope(request *wakutypes.Envelope) *wakutypes.ReceivedMessage {
	if s.asymFilter != nil {
		if d := request.Open(s.asymFilter); d != nil {
			return d
		}
...
func (s *WakuMailServer) decodeRequest(peerID []byte, request *wakutypes.Envelope) (MessagesRequestPayload, error) {
	var payload MessagesRequestPayload
